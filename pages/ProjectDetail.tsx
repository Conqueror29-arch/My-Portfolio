...
import { ArrowLeft, ArrowUpRight, Check, ArrowRight, Layers, Trophy, Github, Globe, Hash, Share2, Twitter, Linkedin, Copy } from 'lucide-react';
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <ScrollReveal animation="fade-up">
          <div className="mb-10">
              ...
              <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-8">
                  <h1 className="text-4xl md:text-6xl lg:text-7xl font-bold text-white font-display uppercase leading-none tracking-tight">
                      {project.title}
                  </h1>
                ...
                <ScrollReveal animation="slide-left" delay={100}>
                  <SpotlightCard className="bg-[#0A0A0A] border border-white/5 p-8 rounded-3xl h-full">
                       ...
                       <div className="flex gap-3">
                          ...
                          <a href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(project.title)}&url=${encodeURIComponent(window.location.href)}`} target="_blank" rel="noopener noreferrer" className="p-4 bg-white/5 rounded-xl border border-white/5 hover:bg-[#1DA1F2] hover:text-white transition-all">
                              <Twitter size={20} />
                          </a>
        ...
        <ScrollReveal animation="fade-up" delay={300}>
          <div className="border-t border-white/10 mt-20 pt-12 flex justify-between items-center group">
              ...
              <Link to={`/works/${nextProject.id}`} className="text-2xl md:text-4xl font-bold uppercase text-white hover:text-primary transition-colors flex items-center gap-4 text-right">
                  {nextProject.title} <ArrowRight size={32} className="group-hover:translate-x-2 transition-transform" />
              </Link>
          </div>
        </ScrollReveal>
