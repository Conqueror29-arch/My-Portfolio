    ...
    <div className={`inline-flex flex-col items-center justify-center align-bottom relative overflow-hidden h-[1.2em] ${className}`}>
      ...
      >
        {words.map((word, i) => {
            ...
            // We want the current word to be at 0, previous at -100%, next at 100%
            // Simple approach: Translate the whole column based on index
            
            return (
                <span 
                    key={i}
                    className="block h-[1.2em] transition-transform duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] text-transparent bg-clip-text bg-gradient-to-r from-primary via-cyan-200 to-primary font-bold tracking-tight whitespace-nowrap"
